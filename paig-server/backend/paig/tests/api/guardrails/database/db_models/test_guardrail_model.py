...
def test_guardrail_config_model_attributes():
    ...
    guardrail_provider_connection_name = "mock_guardrail_connection"
    ...
    guardrail_config_model = GRConfigModel(
        ...
        guardrail_provider_connection_name=guardrail_provider_connection_name,
    ...
    assert guardrail_config_model.guardrail_provider_connection_name == guardrail_provider_connection_name
    ...
    assert guardrail_config_model.guardrail_provider_connection_name is not None
...
def guardrail_config(mock_sqlalchemy_session, guardrail):
    config_instance = GRConfigModel(
        guardrail_id=guardrail.id,
        guardrail_provider=GuardrailProvider.AWS,
        guardrail_provider_connection_name="mock_connection",
        config_type="mock_config_type",
        config_data={"mock_key": "mock_value"}
    )
...
def test_guardrail_has_config(mock_sqlalchemy_session, guardrail, guardrail_config):
    ...
    assert guardrail_from_db.gr_config[0].config_type == "mock_config_type"
...
def test_config_has_guardrail(mock_sqlalchemy_session, guardrail_config):
    ...
    config_from_db = mock_sqlalchemy_session.query(GRConfigModel).filter_by(
        guardrail_provider_connection_name="mock_connection").one()
    ...
    assert config_from_db.guardrail.gr_config[0].guardrail_provider_connection_name == "mock_connection"
    assert config_from_db.guardrail.gr_config[0].config_type == "mock_config_type"
