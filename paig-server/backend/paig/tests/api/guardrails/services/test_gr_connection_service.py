...
async def test_create_guardrail_connection(guardrail_connection_service, mock_guardrail_connection_repository, mock_encryption_key_service):
    mock_gr_connection_view = copy.deepcopy(gr_connection_view)
    with (patch.object(
            mock_guardrail_connection_repository, 'create_record', return_value=mock_gr_connection_view
        ...
        # Call the method under test
        result = await guardrail_connection_service.create(mock_gr_connection_view)

        # Assertions
        assert result == mock_gr_connection_view
...
@pytest.mark.asyncio
async def test_update_guardrail_connection(guardrail_connection_service, mock_guardrail_connection_repository, mock_encryption_key_service):
    mock_gr_connection_view = copy.deepcopy(gr_connection_view)
    with (patch.object(
            mock_guardrail_connection_repository, 'update_record', return_value=mock_gr_connection_view
        ...
        # Call the method under test
        result = await guardrail_connection_service.update(1, mock_gr_connection_view)
