...
@pytest.fixture()
def mock_response_template_view_repository():
    """Fixture for mocking the ResponseTemplateViewRepository."""
    return AsyncMock(spec=ResponseTemplateRepository)


@pytest.fixture
def request_validator(mock_response_template_view_repository):
    """Fixture for initializing the ResponseTemplateRequestValidator."""
    return ResponseTemplateRequestValidator(response_template_view_repository=mock_response_template_view_repository)

@pytest.fixture
def service(mock_response_template_repository, mock_response_template_view_repository, request_validator):
    ...
    return ResponseTemplateService(
        ...
        response_template_view_repository=mock_response_template_view_repository,
        response_template_request_validator=request_validator
    )

@pytest.mark.asyncio
async def test_validate_create_request(request_validator, mock_response_template_view_repository):
    """Test the create request validation."""
    mock_request = ResponseTemplateView(response="Test Response", description="Test Description")
    mock_response_template_view_repository.list_records.return_value = ([], 0)

    await request_validator.validate_create_request(mock_request)
    mock_response_template_view_repository.list_records.assert_called_once()

@pytest.mark.asyncio
async def test_validate_create_request_existing_response(request_validator, mock_response_template_view_repository):
    ...
    mock_response_template_view_repository.list_records.return_value = ([existing_template], 1)

    with pytest.raises(BadRequestException):
        await request_validator.validate_create_request(mock_request)

@pytest.mark.asyncio
async def test_list_response_templates(service, mock_response_template_view_repository):
    ...
    mock_response_template_view_repository.list_records.return_value = ([ResponseTemplateModel(id=1, response="Test Response", description="Test")], 1)
...
@pytest.mark.asyncio
async def test_create_response_template(service, request_validator, mock_response_template_repository, mock_response_template_view_repository):
...
@pytest.mark.asyncio
async def test_update_response_template(service, request_validator, mock_response_template_repository, mock_response_template_view_repository):
