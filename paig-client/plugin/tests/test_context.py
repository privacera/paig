...
def set_user_in_context(user):
    ...
    privacera_shield.client.set_current_user(user)
    for i in range(10):
        time.sleep(0.1)
        current_user = privacera_shield.client.get_current_user()
        print(
...
def test_user_context_threads(setup_paig_plugin_with_app_config_file_name):
    privacera_shield.client.setup(application_config_file=setup_paig_plugin_with_app_config_file_name, frameworks=["langchain"])
    thread_list = []
    for i in range(10):
