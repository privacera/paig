...
function build_gitlab_pyarmor_all_platforms() {
  ...
  for curr_platform in "${platform[@]}"; do
    ...
    pyarmor gen \
      ...
      -i plugin/src/privacera_shield

    find .pyarmor -ls
    grep trace.rft .pyarmor/pyarmor.trace.log
    grep trace.bcc .pyarmor/pyarmor.trace.log

    cp plugin/{LICENSE,README.md,pyproject.toml,privacera_version.txt} ${DIST_PYARMOR_DIR}
...
function rename() {
  pattern="dist/privacera_shield-*py3-none-any.whl"
